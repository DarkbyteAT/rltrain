...
class StreamLogger:
    ...
    def __init__(self, stream: IO[str] | None = None) -> None:
        """Store the output stream, defaulting to ``sys.stdout``."""
        self._stream = stream or sys.stdout

    def start(self, config: dict[str, Any], run_dir: Path) -> None:
        """See ``MetricsLogger.start``; no setup required for stream logging."""
        ...

    def log_scalars(self, metrics: dict[str, float], step: int) -> None:
        """Write a ``[step N] key=value ...`` line to the stream."""
        parts = " ".join(f"{k}={v:.4g}" for k, v in metrics.items())
        self._stream.write(f"[step {step}] {parts}\n")
        self._stream.flush()

    def log_hyperparams(self, params: dict[str, Any]) -> None:
        """See ``MetricsLogger.log_hyperparams``; not implemented for stream logging."""
        ...

    def finish(self) -> None:
        """See ``MetricsLogger.finish``; no teardown required for stream logging."""
        ...
