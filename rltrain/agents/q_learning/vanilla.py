...
class VanillaDQN(Agent):
    ...
    def act(self, states: T.Tensor) -> dst.Distribution:
        ...
        return dst.Categorical(
            probs=T.where(
                F.one_hot(opt_actions, action_space).bool(), 1 - self.eps_greedy, self.eps_greedy / (action_space - 1)
            )
        )
    ...
    def load(self) -> tuple[T.Tensor, ...]:
        ...
        arrays = [np.asarray(x).squeeze() for x in zip(*batch, strict=False)]
        result = []
