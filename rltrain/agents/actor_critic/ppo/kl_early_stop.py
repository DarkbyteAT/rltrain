...
class KLEarlyStop:
    ...
    def __init__(self, target_kl: float, rollback: bool = True):
        """Initialize the KL early-stop terminator.

        Args:
            target_kl: KL divergence threshold. When the epoch's approximate KL
                exceeds this value, ``should_stop`` returns ``True``.
            rollback: If ``True``, PPO restores parameters to their pre-epoch
                state when stopping (KLE-Rollback). If ``False``, parameters
                stay as-is (KLE-Stop).
        """
    ...
    def should_stop(self, approx_kl: float) -> bool:
        """Return ``True`` if ``approx_kl`` exceeds the configured threshold."""
