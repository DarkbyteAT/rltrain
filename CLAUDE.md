...
- **Trainer + Loops**: `Trainer.fit(key)` auto-detects loop strategy from `env.capabilities`. Four impls behind the `TrainingLoop` Protocol: `PythonLoop`, `ScanLoop`, `PmapLoop` (single-device fallback to ScanLoop), and `MultiSeedScanLoop` (dispatched by the sibling `MultiSeedTrainer`, which takes an `agent_factory` and fits `n_seeds` independently-initialised agents in parallel via `eqx.filter_vmap` over the seed axis).
